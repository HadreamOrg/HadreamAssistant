...
class HACamera:
    ...
    def capture_image_cv(self, fp="./backend/data/image/capture.jpg", display=True):
        ...
        :return: img_data
        """
        self.log.add_log("HACamera: capturing one image using opencv2 and openni", 1)

        a, img = self.cv2_camera.read()
        cv2.imwrite(fp, img)
    ...
    def capture_video(self, fp="./backend/data/video/video.avi", size=None, fps=None):
        ...
        while self.cv2_camera.isOpened():
