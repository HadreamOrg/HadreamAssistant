...
class HANLU:
    ...
    """
    intent:
        意图的表述为一个tuple，第一位为技能，第二位为技能函数 
        意图为list时，是不确定状态，有备选，需要确定；
    """
    ...
    def analyze_intent(cls, text):
        ...
        if "打开" in text:
            ...
            try:
                ...
                cls.log.add_log("detect open_skill, skill_called-%s" % skill_called, 0)
                skill_name = None
                for a in cls.skill_manager.name_skills_list:
                    for skill_name_group in a:
                        if skill_called in skill_name_group:
                            # compared
                            skill_name = a[-1]
                            cls.log.add_log("open_skill name compared, skill-%s" % skill_name, 1)
                            break
                    if skill_name is not None:
                        break
            except KeyError:
                cls.log.add_log("skill_name-%s does not exist, open_skill fail" % text[2:], 3)
                nlu_result = ["error", None, "skill_name_not_exist", None]
            else:
                nlu_result[2] = skill_name
        else:
            # 原生意图识别（包含词槽）
            group_last_one_index = len(HASkillManager.keyword_intent_list)
            for skill_intent_group_index in range(0, group_last_one_index):
                ...
                skill_group = cls.skill_manager.keyword_intent_list[skill_intent_group_index]
                ...
                for i in range(0, len(skill_group[0])):
                    ...
                    if intent_compared:
                        # 第i个意图已经被匹配
                        # intent[2] = group[2]
                        cls.log.add_log("skill intent compared, %s-%s" % (skill_group[-1], skill_group[1][i]), 1)
                        if multiple_intents:
                        ...
                        # 开始解析词槽
                        slot_result = cls.get_slot(text, [slot_group])
                        break
        ...
        cls.log.add_log("analyze result: %s" % nlu_result, 0)
        return nlu_result

    @classmethod
    def analyze_skill_intent(cls, text, skill_name, skill_intent_list):
        ...
        for i in range(0, len(skill_intent_list[0])):
            ...
            if intent_compared:
                # 第i个意图已经被匹配
                cls.log.add_log("skill_intent-%s compared" % skill_intent_list[1][i], 1)
                if multiple_intents:
                ...
                # 开始解析词槽
                nlu_result[-1] = cls.get_slot(text, [slot_group])
                break

        return nlu_result

    @classmethod
    def get_slot(cls, text, slots):
        ...
        """
        cls.log.add_log("start analyzing slots...", 1)
        slot_result = {}

        nlp_result = cls.nlp.lexer(text)
        nlp_result = cls.nlp.lexer_result_process(nlp_result)
        ...
        for slot in slots:
            ...
            if "$" in slot[1]:
                # dict mode
                slot_dict = json.load(open(r"./data/json/skill_slots/dict_%s.json" % slot[1].replace("$", "").replace("!", "")))
                if slot_dict["canNlp"]:
                    # nlp法
                    cls.log.add_log("start analyzing slot-%s in nlp mode" % slot_name, 1)
                    try:
                        slot_result[slot_name].append(nlp_result[slot_dict["nlpItemType"]][slot_dict["nlpItemType2"]])
                    except KeyError:
                        cls.log.add_log("cannot resolve slot from nlp's preset", 1)
                        if ask:
                            cls.log.add_log("slot-%s not compared and asking is available, request ask_slot" % slot_name, 1)
                            slot_result[slot_name] = cls.ask_slot([slot])[slot_name]
                        else:
                            cls.log.add_log("asking slot is not required, skip the slot", 1)
                else:
                    # 词匹配法
                    ...
                    for content_group in slot_dict_content:
                        ...
                        if "$" in real_word:
                            # 引用了字典文件，加载字典文件
                            content_group = json.load(open(r"./data/json/skill_dicts/%s.json" % real_word.replace("$", ""), "r", encoding="utf-8"))
                            real_word = content_group[0]
                ...
                # rule mode
                slot_rule = json.load(open(r"./data/json/skill_slots/rule_%s.json" % slot[1].replace("*", "").replace("!", "")))

                cls.log.add_log("start analyzing slot-%s in rule_%s mode" % (slot_name, slot_rule["mode"]), 1)
                mode = slot_rule["mode"]
                slot_compared = False
                ...
                if mode == "context_mode":
                            ...
                            if (next_word is None or next_word == "") and length_group is not None:
                                ...
                                if length_group is not None:
                                    if len(word) != length_group[i]:
                                        cls.log.add_log("word's length not matched, skip", 3)
                                        continue
                                    else:
                                        slot_compared = True
                        except ValueError:
                            cls.log.add_log("last/next_word was not found in text(context_mode), continue", 2)
                            continue
                        else:
                    ...
                    cls.log.add_log("rule_mode-mode-%s does not supported" % mode, 2)
                    continue
    ...
    def ask_slot(cls, slots_param, retried_limit=2):
        ...
        STEPS:
            ...
            2.
        """
        cls.log.add_log("start asking slot", 1)
        ...
        for slot in slots_param:
            ...
            asking_text = slot[-1]
            result[slot_name] = []
            cls.log.add_log("now asking slot-%s" % slot_name)

            if "!" in slot[1]:
                cls.log.add_log("force ask is not allowed in ask_slot, skip", 2)
                continue
            ...
            while True:
                if text is None:
                    if retried > retried_limit:
                        cls.log.add_log("ask for slot-%s failed, text is empty, skip(over limit)" % slot[0], 3)
                        break
                    else:
                        retried += 1
                        cls.log.add_log("text is empty, retry...", 2)
                        text = ask()
                else:
                    nlu_result = cls.get_slot(text, [(slot_name, slot[1])])
                    try:
                        result[slot_name].append(nlu_result[slot_name])
                    except KeyError:
                        cls.log.add_log("HASkillNotion: ask for place fail, can't get the slot, skip", 2)
                        continue
                    break
